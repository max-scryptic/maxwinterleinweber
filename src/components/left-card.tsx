 ...
 *
 * What it is for is the clear end of the slider. The name is the only text on
 * the card that sits on bare glass, so at 0.4 fill it is read against whatever
 * the cloud is doing behind it, and that is not a fixed background but a
 * swinging one: black on the plate measures about 3.5:1 where the empty sky
 * passes behind and about 11.6:1 where a lit filament does. The hairlines put a
 * value break hard against the edge of every stroke, so the letter has an edge
 * of its own to be read by wherever the cloud happens to be.
 *
 ...
 * nothing to show, and what is left is the name black on near white.
 */
